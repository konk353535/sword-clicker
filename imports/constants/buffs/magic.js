...
export const MAGIC_BUFFS = {
    ...
    water_wave: {
        ...
        events: {
            ...
            onApply({ buff, target, caster, actualBattle }) {
                const constants =
                    buff && buff.constants && buff.constants.constants
                        ? buff.constants.constants
                        : lookupBuff("water_wave").constants
  ...
  */
    ...
    charm: {
        ...
        events: {
            // This can be rebuilt from the buff id
            onApply({ buff, target, caster, actualBattle }) {
                const hasCantBeCharmedOrCharmed = target.buffs.find(
                    (buff) => (buff.id === "charm" && buff.data.wasCharmed) || buff.id === "cant_be_charmed"
                )

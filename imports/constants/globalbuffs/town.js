export const TOWN_GLOBALBUFFS = Object.freeze({
    ...
    town_quarry: {
        ...
        effects: function (level = 1) {
            return [`+${(level + 1) * 3}% extra passive miner damage`, `+${(level + 1) * 5}% faster mine pit deposits`]
        },
        isServerBuff: true,
        hasLevels: true
    },
    ...
    town_lumber_yard: {
        ...
        effects: function (level = 1) {
            if (level === 1) {
                return [`+${(level + 1) * 2.5}% faster woodcutters`]
            }
            return [
    ...
    town_armory: {
        ...
        effects: function (level = 1) {
            return [
                ...
                `${level * 5}% better chance to reforge equipment (must still qualify to reforge to get this bonus)`
            ]
        },
        isServerBuff: true,
        hasLevels: true
    ...
    town_observatory: {
        ...
        effects: function (level = 1) {
            return [`+${(level + 1) * 2.5}% Astronomy XP`, `+${(level + 1) * 3}% astronomy mage shards/hour rate`]
        },
        isServerBuff: true,
        hasLevels: true
    }

...
Template.body.onCreated(function () {
  ...
  */
    ...
    const startupSync = function() {
        if (Meteor.user()) {
            Meteor.call("combat.syncCombatStats")
            Meteor.call("classes.checkAbilities")
        } else {
            Meteor.setTimeout(startupSync, 5000)
        }
    }
...
Template.registerHelper("math", function () {
    ...
    return {
        mul(...terms) {
            return result(terms, (a, b) => a * b)
        },
        div(...terms) {
            return result(terms, (a, b) => (b ? a / b : 0))
        },
        sum(...terms) {
            return result(terms, (a, b) => a + b)
        },

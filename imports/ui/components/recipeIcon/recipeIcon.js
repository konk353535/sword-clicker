...
const updateCraftable = function (instance) {
    ...
    let { recipeItems, hasConsumeItemRequirements, consumeItemRequirements, maxCraftable, notMet } =
        determineRequiredItems(recipe)
                ...
                Meteor.call("abilities.getAbilityInfo", abilityId, level, (res, data) => {
                    ...
                    if (abilityConstants && abilityConstants.buffs && abilityConstants.buffs[0]) {
                        ...
                        instance.state.set(
                            ...
                            `At level <b>${level}</b>:<br />${data.description}`
                        )
                    }
                })
            }
...
Template.recipeIcon.events({
    ...
    "click .quick-craft"(event, instance) {
        if (instance.$(".recipe-tooltip").css("opacity") > 0.9) {
            ...
            const amountToCraft = parseInt(
                instance.$(event.target).closest(".quick-craft")[0].getAttribute("data-amount")
            )
            const recipeConstants = instance.data.recipe
            if (amountToCraft <= 0) {
                return
            ...
            Meteor.call(
                `${isInscription ? "inscription" : "crafting"}.craftItem`,
                recipeId,
                amountToCraft,
                (err, res) => {
                    if (err) {
                        if (err.errorType === "Meteor.Error" && err.isClientSafe && err.reason) {
                            toastr.warning(
                                `Failed to ${isInscription ? "inscribe" : "craft"} ${recipeConstants.name}. ${
                                    err.reason
                                }`
                            )
                        } else {
    ...
    "click .craft-btn"(event, instance) {
        ...
        Meteor.call(`${isInscription ? "inscription" : "crafting"}.craftItem`, recipeId, amountToCraft, (err, data) => {
            if (err) {
                if (err.errorType === "Meteor.Error" && err.isClientSafe && err.reason) {
                    toastr.warning(
                        `Failed to ${isInscription ? "inscribe" : "craft"} ${recipeConstants.name}. ${err.reason}`
                    )
                } else {
                    console.log(err)
                    toastr.warning(`Failed to ${isInscription ? "inscribe" : "craft"} ${recipeConstants.name}`)

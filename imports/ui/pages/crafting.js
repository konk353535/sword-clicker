...
import { CInt, CDbl, IsValid } from '/imports/utils.js';
  ...
  items() {
    ...
    if (itemViewLimit !== 0) {
      if (bShowHidden && bShowUnhidden) {
        return ItemSorter(FetchSomeVisibleItems(highestFurnaceTier, itemViewLimit).concat(FetchSomeHiddenItems(highestFurnaceTier, itemViewLimit)));
      } else if (bShowHidden) {
  ...
  try {
      ...
      if (recipes !== undefined) {
          recipes.forEach((currentRecipe, index, array) => {
              ...
              if (currentRecipe.name.trim().toLowerCase() === RecipeSearch)
                  RecipeResult = currentRecipe;
          });
      }
  }
  catch (err) {
...
const ItemSorter = function(itemList_in) {
  ...
  let itemList = itemList_in.map((item) => {

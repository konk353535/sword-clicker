...
Template.overviewPage.onCreated(function bodyOnCreated() {
    ...
    const updateAdventures = function (self) {
        ...
        self.state.set(
            "adventures",
            self.state.get("rawAdventures").map((adventure) => {
                if (adventure.duration) {
                    adventure.durationTotalDisplay = moment("2015-01-01")
                        .startOf("day")
                        .seconds(adventure.duration)
                        .format("H:mm:ss")
                }
                ...
                if (
                    ...
                    moment().isAfter(adventure.startDate) &&
                    moment().isBefore(adventure.endDate)
                ) {
                    // Seconds since start
                    ...
                    const totalSeconds = moment
                        .duration(moment(adventure.endDate).diff(adventure.startDate))
                        .asSeconds()
...
Template.overviewPage.helpers({
    ...
    friendsList() {
        ...
        if (friendsListState && friendsListState.friends) {
            return friendsListState.friends.sort((a, b) => {
...
Template.friendRow.helpers({
    ...
    canInvite() {
        ...
        return !currentGroup || (currentGroup.leader === Meteor.userId() && currentGroup.members.length < 5)
    }
})

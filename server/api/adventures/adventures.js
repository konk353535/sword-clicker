...
import { addItem, consumeGems, consumeItem, hasGems } from "/server/api/items/items.js"
    ...
    if (adventure.type === "physical") {
        ...
        if (hasCombatGlobalBuff) {
            adventure.rewards.push({
                type: "xp",
                amount: Math.round(totalXp * 0.33 * completionDecimal * 0.2),
                skill: "attack",
        ...
        if (hasCombatGlobalBuff) {
            adventure.rewards.push({
                type: "xp",
                amount: Math.round(totalXp * 0.03 * completionDecimal * 0.2),
                skill: "magic",
                affectedGlobalBuff: true
            })
            adventure.rewards.push({
                type: "xp",
                amount: Math.round(totalXp * 0.97 * completionDecimal * 0.2),
                skill: "health",
                affectedGlobalBuff: true
            })
        }
...
Meteor.methods({
    "adventures.cancelAdventure"(adventureId) {
        ...
        const sortedActiveAdventures = _.sortBy(
            myAdventures.adventures.filter((adventure) => {
    ...
    "adventures.startAdventure"(adventureId) {
        ...
        if (activeAdventures.length >= MAX_ACTIVE_ADVENTURES) {
            throw new Meteor.Error(
                "too-many-adventures",
                `Only a maximum of ${MAX_ACTIVE_ADVENTURES} adventures queued as once`
            )
        }

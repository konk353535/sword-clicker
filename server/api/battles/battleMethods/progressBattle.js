...
import { ABILITIES } from '/server/constants/combat/index.js'; // List of available combat stats
  ...
  try {
    ...
    actualBattle.units.forEach((unit) => {
      if (unit && (actualBattle.tick - unit.tickOffset) % unit.stats.attackSpeedTicks === 0) {
        ...
        if (unit.target) {
          const targetEnemy = _.find(actualBattle.enemies, (enemy) => {
            return enemy.id === unit.target
          });
          if (targetEnemy) {
    ...
    battleActions.forEach((action) => {
      if (action.abilityId === 'changeTarget') {
        ...
        const targetUnit = _.find(actualBattle.enemies, (enemy) => {
          return enemy.id === action.targets[0];
        });
        const casterUnit = _.find(actualBattle.units, (unit) => {
          return unit.id === action.caster;
        });
        ...
        for (let i = 0; i < castersUnits.length; i++) {
          unitAbility = _.find(castersUnits[i].abilities, (ability) => {

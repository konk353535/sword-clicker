...
Meteor.methods({
    "battles.killBattle"() {
        ...
        try {
            ...
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mbattles.killBattle\x1b[22m"()\x1b[0m`)
    ...
    "battles.findPersonalBattle"(level, energyUse) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mbattles.findPersonalBattle\x1b[22m"(${level}, ${energyUse})\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
    ...
    "battles.findTowerBattle"(floor, room, energyUse) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mbattles.findTowerBattle\x1b[22m"(${floor}, ${room}, ${energyUse})\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }

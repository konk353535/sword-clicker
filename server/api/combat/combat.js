...
export const updateCombatStats = function (userId, username, amuletChanged = false, newClass = undefined) {
    ...
    for (let i = 0; i < combatItems.length; i++) {
        ...
        if (combatItem.constants.slot === "mainHand") {
            ...
            playerData.offHandIsMagic = combatItem.constants.isMagic
        }
...
Meteor.methods({
    ...
    "combat.changeClass"(classId) {
        ...
        try {
            ...
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mcombat.changeClass\x1b[22m"("${classId}")\x1b[0m`)
    ...
    "combat.updateCharacterIcon"(id) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mcombat.updateCharacterIcon\x1b[22m"("${id}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }

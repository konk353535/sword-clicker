...
Meteor.methods({
    ...
    "groups.invite"(username) {
        ...
        if (
            currentGroup &&
            (_.contains(currentGroup.invites, targetUser._id) || _.contains(currentGroup.members, targetUser._id))
        ) {
            return
        }
        ...
        if (currentGroup && currentGroup.members.length + currentGroup.invites.length + 1 > BATTLES.maxBossPartySize) {
            throw new Meteor.Error(
                "group-full",
                `Group is full (${currentGroup.members.length + currentGroup.invites.length} / ${
                    BATTLES.maxBossPartySize
                }) members`
            )
        }
    ...
    "groups.create"() {
        ...
        if (currentGroup) {
            throw new Meteor.Error("group-exists", `You are already in a group!`)
        }

...
Meteor.methods({
    "items.unequip"(_id, itemId) {
        ...
        try {
            ...
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.unequip\x1b[22m"("${_id}", "${itemId}")\x1b[0m`)
    ...
    "items.use"({ baseItemId, targetItemId }) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.use\x1b[22m"("${baseItemId}", "${targetItemId}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
        ...
        if (baseItem.itemId === "enhancer_key") {
            ...
            if (IsJewelAmulet(targetItem)) {
                if (
                    (targetItem.extraStats && targetItem.extraStats.level && targetItem.extraStats.level >= 9) || // Amulet Upgrade Limit
                    targetItem.enhanced
                ) {
            ...
            if (targetItemConstants.extraStats && targetItem.quality < 100 && !targetItem.enhanced) {
                ...
                if (currentQualityIncrease > 0) {
                    ...
                    extraStatKeys.forEach((extraStatKey) => {
                        ...
                        if (maxValue !== 0) {
                            // shouldn't be possible since .filter() earlier catches this, but just to be safe
        ...
        } else if (baseItem.itemId === "phasing_key") {
            if (targetItemConstants.extraStats) {
                ...
                addItem(targetItemConstants.id, 1, Meteor.userId())
            } else {
                throw new Meteor.Error("invalid-target", "Invalid target item")
            }
            ...
            if (!crafting.learntCrafts[baseItemConstants.teaches]) {
                ...
                if (baseItemConstants.teachesDuration) {
                    crafting.expiringCrafts[baseItemConstants.teaches] = moment()
                        .add(baseItemConstants.teachesDuration, "days")
                        .toDate()
                }
    ...
    "items.eat"(_id, itemId) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.eat\x1b[22m"("${_id}", "${itemId}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
    ...
    "items.findAndConsume"(uid, itemId) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.findAndConsume\x1b[22m"("${uid}", "${itemId}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
    ...
    "items.unequipAllCombat"() {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.unequipAllCombat\x1b[22m"()\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
    ...
    "items.equip"(_id) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.equip\x1b[22m"("${_id}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
    ...
    "items.sellItem"(_id, itemId, amount) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mitems.sellItem\x1b[22m"("${_id}", "${itemId}", ${amount})\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-blocked", "That action is blocked; please contact support.")
                }
            }
...
export const UseItemBox = function (baseItem, baseItemConstants) {
    ...
    if (chosenItem.indexOf("gold:") === 0) {
        ...
        sendUserChatMessage({
            ...
            message: `Inside the ${baseItemConstants.name}, you discovered:  ${Numeral(amountInBox).format("0,0")} gold`
        })
        ...
        sendUserChatMessage({
            userId: Meteor.userId(),
            message: `Inside the ${baseItemConstants.name}, you discovered:  ${Numeral(amountInBox).format("0,0")} gems`
        })
...
export const UseJade = function (baseItem, baseItemConstants, targetItem, targetItemConstants) {
    ...
    const level =
        targetItem.extraStats.level <= 5
            ? targetItem.extraStats.level
            : 4 + Math.sqrt((targetItem.extraStats.level - 5) * 2.5)
...
export const UseTanzanite = function (baseItem, baseItemConstants, targetItem, targetItemConstants) {
    ...
    const attackRate = 1.3
    const attackMaxRate = 1.3
    const accuracyRate = 1.3
    const defenseRate = 1.3
    const healthMaxRate = 1.2
    const magicPowerRate = 1.5
    const magicArmorRate = 1.3

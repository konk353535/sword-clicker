...
export const updateMiningStats = function (userId, slot = "pickaxe", isNewUser = false) {
    ...
    if (!miningStats.attack) {
        miningStats.attack = 1
    }
    if (!miningStats.energyStorage) {
        miningStats.energyStorage = 10
    }
    if (!miningStats.energyRegen) {
        miningStats.energyRegen = 10
    }
    if (!miningStats.energyPerHit) {
        miningStats.energyPerHit = 1
    }
...
Meteor.methods({
    ...
    "mining.fetchMiners"() {
        ...
        const minersArray = Object.keys(MINING.miners)
            .map((key) => {
    ...
    "mining.fetchOres"() {
        ...
        const oresArray = Object.keys(MINING.ores)
            .map((key) => {
    ...
    "mining.fetchProspectors"() {
        ...
        const prospectorsArray = Object.keys(MINING.prospectors)
            .map((key) => {

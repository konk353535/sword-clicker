...
import { calculateItemKarma, createTown, karmaLevelValues } from "/imports/api/town/town.js"
...
const donatedItemExistsData = function donatedItemExistsData(inputGoods, inputItem) {
    ...
    for (let i = 0; i < keys.length; i++) {
        if (
            inputGoods[i].itemId === inputItem.itemId &&
            inputGoods[i].owner === inputItem.owner &&
            inputGoods[i].townBuilding === inputItem.townBuilding &&
            inputGoods[i].rarityId === inputItem.rarityId
        ) {

...
Meteor.methods({
    "woodcutting.fireWoodcutter"(index) {
        ...
        try {
            ...
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mwoodcutting.fireWoodcutter\x1b[22m"(${index})\x1b[0m`)
    ...
    "woodcutting.hireWoodcutter"(woodcutterId) {
        ...
        try {
            const logUserDoc = Meteor.user()
            if (logUserDoc) {
                console.log(`\x1b[33m[API] ${logUserDoc.username} ${logUserDoc.clientIp} "\x1b[1mwoodcutting.hireWoodcutter\x1b[22m"("${woodcutterId}")\x1b[0m`)
                if (logUserDoc.xpActionsBlocked) {
                    throw new Meteor.Error("action-locked", "That action is blocked; please contact support.")
                }
            }

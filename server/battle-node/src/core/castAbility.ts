...
export function castAbility(this: Battle, { ability, caster, targets }: castAbilityOpts) {
    ...
    if (HasBowOrQuiverButNotTheOther && ability.requires) {
        ability.requires.forEach((required: any) => {
            let meetsRequirement = true
            if (required.type === "weaponType") {
                required.weaponTypes.forEach((weaponType: any) => {
    ...
    if (canUseAbilityOrSpell && ability.cantUseWith) {
        ability.cantUseWith.forEach((cantUseWithThis: any) => {
            let meetsRequirement = true
            if (cantUseWithThis.type === "weaponType") {
                cantUseWithThis.weaponTypes.forEach((weaponType: any) => {
    ...
    if (ability.target === "currentEnemy" || ability.target === "singleEnemy") {
        ...
        if (!target.stats || !target.stats.health || target.stats.health <= 0) {
            return false // can't cast on a target enemy without a valid target enemy unit (returning false will prevent cooldown, resource expenditure, etc.)
        }
    }
    ...
    if (ability.id === "scream" && targets.length > 0) {
        ...
        ability.cdAdjust = function (abil: Ability) {
            let maxCD = 9999
            if (ability.level >= 2) {
    ...
    targets.forEach((target) => {
        const newBuffs = ability.buffs.map((buffId: string) => {
            const buffObj: any = {}
            // Store constants
            ...
            buffObj.data = Object.assign(
                {}, 
                {
                    description: buffObj.constants.description({
                        ...
                        level: ability.level,
                        playerSkills: caster.skills,
                        floor: caster.battleRef.floor
                    }),

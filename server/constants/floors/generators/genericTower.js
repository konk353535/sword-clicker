...
export const genericTowerMonsterGenerator = function(floor, room) {
  ...
  allMonsters.forEach((selectedMonsterId) => {
    ...
    const monster = {
      ...
      stats: {
        health: (room / 1.2) * 25 * floor * (1 + (floor / 3)) * (1 / totalUnits),
        healthMax: (room / 1.2) * 25 * floor * (1 + (floor / 3)) * (1 / totalUnits),
        ...
        accuracy: (room / 2.4) * 4 * floor * (1 + (floor / 3)),
        armor: (room / 2.4) * 25 * (floor / 4),
        defense: (room / 2.2) * 1.8 * floor * (1 + (floor / 3)),
